...
const TEMPLATE_CHECKLIST: { mesesAntes: number; titulo: string; categoria: string }[] = [
  // 12 meses antes
  {
    mesesAntes: 12,
  ...
  // 1 semana antes (mesesAntes 0 = tratado como 7 dias antes no cálculo do prazo)
  {
    mesesAntes: 0,
...
async function seed() {
  ...
  await db.insert(tasks).values(
    TEMPLATE_CHECKLIST.map((item, index) => {
      const prazo = calcularPrazo(item.mesesAntes)
      const concluida = prazo < HOJE && index % 5 !== 0
      return {
        weddingId: wedding.id,
        titulo: item.titulo,
        categoria: item.categoria as (typeof tasks.$inferInsert)["categoria"],
  ...
  const percentuais: [string, number, string][] = [
    ["Buffet", 30, "#6f7350"],
    ["Local", 15, "#a8442e"],
    ["Fotografia e vídeo", 10, "#b89b5e"],
    ["Decoração e flores", 10, "#7f8a99"],
    ["Vestuário e beleza", 10, "#9c8567"],
    ["Música", 8, "#8a6f9c"],
    ["Bolo e doces", 4, "#c98a6b"],
    ["Convites e papelaria", 4, "#5e8a7a"],
    ["Cerimonial", 4, "#a67f8e"],
    ["Lembrancinhas", 3, "#8f9c5e"],
    ["Outros", 2, "#8a8378"],
  ]
  const categoriasCriadas = await db
    .insert(budgetCategories)
    .values(
      percentuais.map(([nome, percentual, cor], ordem) => ({
        weddingId: wedding.id,
        nome,
